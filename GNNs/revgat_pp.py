...
class RevGAT(nn.Module):
    def __init__(
        ...
        for i in range(self.n_layers):
            perm = torch.randperm(number_of_edges)
            self.perms.append(perm)
        
        m = torch.zeros(169343, n_heads * n_hidden).bernoulli_(1 - self.dropout)
        self.mask = m.requires_grad_(False) / (1 - self.dropout)

    def forward(self, graph, feat):
        h = feat
        for perm_itm in self.perms:
            perm_itm = perm_itm.to(h.device)
        ...
        if self.first_stage:
            ...
            for i in range(1, len(self.convs)):
                ...
                h = self.convs[i](h, graph, self.mask, perm)
