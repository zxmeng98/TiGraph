...
layers=448
hidden=80
torchrun --nnodes 1 --nproc_per_node 4 revgnn_pp_trainonall.py --dataset $dataset --num_layers $layers --hidden_channels $hidden --epochs 1000 --dropout 0.1 --lr 0.001 --bs 2708 --mb_size 2708 >> logs/${layers}-${hidden}revgcn_${dataset}_acc.log

torchrun --nnodes 1 --nproc_per_node 4 revgnn_pp_trainonall_sync_lm_reorder.py --dataset ogbn-arxiv --num_layers 112 2>&1 | tee logs/revgcn_arxiv_sync_lm_acc.log
...
layers=40
hidden=768
torchrun --nnodes 1 --nproc_per_node 4 revgat_pp_trainonall.py --dataset $dataset --num_layers $layers --hidden_channels $hidden --bs 2708 --mb_size 2708 --dropout 0.1 --lr 0.001 >> logs/${layers}-${hidden}revgat_${dataset}_sync_lm_acc.log
...
layers=112
hidden=64
torchrun --nnodes 1 --nproc_per_node 4 --master_port 2923 resgnn_pp_trainonall_sync_lm_reorder.py --dataset $dataset --num_layers $layers --hidden_channels $hidden --dropout 0.5 --lr 0.01 --bs 19717 --mb_size 19717 --epochs 500 >> logs/${layers}-${hidden}resgen_${dataset}_sync_lm_acc.log
