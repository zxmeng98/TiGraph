...
class DebertaEncoder(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.layer = nn.ModuleList([DebertaLayer(config) for _ in range(config.num_hidden_layers)]) # 12
...
class DebertaModel(DebertaPreTrainedModel):
    ...
    def forward(
        ...
        if exit_layer is not None and 0 <= exit_layer < len(encoded_layers):
            sequence_output = encoded_layers[exit_layer]
        else:
            if self.z_steps > 1:
