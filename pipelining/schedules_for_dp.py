...
class _PipelineSchedule(ABC):
    ...
    def _compute_loss_subset(self, output, target, split_idx, chunk_sg_node_idx):
        ...
        """
        chunk_sg_node_idx = chunk_sg_node_idx.cpu().numpy()
        mapper = {node: idx for idx, node in enumerate(chunk_sg_node_idx)} # original node idx -> chunked sub graph node idx
    ...
    def _split_inputs(
        ...
        """
        if args or kwargs:
            args_split, kwargs_split, chunkg_ori_node_idxes = split_args_kwargs_into_chunks(
                args,
                kwargs,
            ...
            return [()] * self._n_microbatches, [{}] * self._n_microbatches, [] * self._n_microbatches
...
def _sorted_batch_p2p(
    ...
    # print(ops_by_peer, len(ops_by_peer))
    # exit(0)
...
class PipelineScheduleSingle(_PipelineSchedule):
    ...
    def step(self, *args, target=None, losses: Optional[List] = None, split_idx: torch.Tensor = None, **kwargs):
        ...
        # Run microbatches
        self._step_microbatches(args_split, kwargs_split, targets_split, losses, split_idx, chunked_sg_ori_node_idxes)
