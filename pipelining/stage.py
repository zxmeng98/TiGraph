...
class _PipelineStageBase(ABC):
    ...
    def _get_recv_ops(
        ...
        for info in recv_infos:
            ...
            peer_rank = self.stage_index_to_group_rank[info.source] # the source rank
            peer_global_rank = (
                peer_rank
    ...
    def get_bwd_send_ops(self, bwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        if self.grad_send_info is None:
            ...
            self.grad_send_info = self._create_grad_send_info(self.args_recv_info[0]) # 0指第一个chunk在每个stage的recv_info，这里只需要每个stage需要send to的stage的index，所以取第一个chunk的recv_info里面的source就可以了
        
        if len(self.grads_input) > len(self.grad_send_info):
            copied_send_stage = self.grad_send_info[0]
            self.grad_send_info = [copied_send_stage for _ in range(len(self.grads_input))]
            # self.grad_send_info.append(self.gra d_send_info[0])
    ...
    def forward_one_chunk(
        ...
        if self.is_first:
            ...
            composite_args = self._retrieve_recv_activations(fwd_chunk_id) # Tuple[tensor, ...]
            # GNN specific: input should add graph data. 
            composite_args = args + composite_args # tuple + tuple 
            # composite_args = composite_args + args # revgnn
            composite_kwargs = {}
        ...
        self._validate_fwd_outputs(output_tuple) # avoid commu hangs
        return output
    ...
    def backward_one_chunk(
        ...
        if self.is_last:
            ...
            grads_output = self._retrieve_recv_grads(bwd_chunk_id) # tensor/Tuple
...
class PipelineStage(_PipelineStageBase):
    ...
    def __init__(
        ...
        # NOTE: 不管哪个stage，self.inputs的格式都要按照最开始进入forward的args来，但是h是从中间状态开始。后续的self.inputs指定都只能是tensor格式
        self.inputs = _create_empty_tensors(input_args[1], device) # gcn
        # self.inputs = _create_empty_tensors(input_args[0], device) # revgnn

        if output_args is None:
            logger.info("output_args not provided, performing forward using input_args")
            self.outputs = self.submod(input_args[0].to(device), *self.inputs) # gcn
            # self.outputs = self.submod(self.inputs[0], input_args[1]) # revgnn
